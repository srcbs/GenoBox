...
def start_bamstats(args, bam, partition, logger, wait=True):
   ...
   if args.mapdamage:
      mapdamage_moab = Moab(mapdamage_calls, logfile=logger, runname='run_genobox_mapdamage', queue=args.queue, cpu=cpuUV, partition=partition)
   else:
      ...
      saturation_moab = Moab(saturation_calls, logfile=logger, runname='run_genobox_saturation', queue=args.queue, cpu=cpuC, partition=partition)
